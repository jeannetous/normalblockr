...
zimvn <- normal_diag_zi$new(data) ; zimvn$optimize()
R <- data$Y - zimvn$fitted ; Rho <- zimvn$model_par$rho
cl0_hclust       <- cutree(hclust(dist(1-cor(R)), method="ward.D2"), 10)
zimvnb_hclust    <- normal_block(data, blocks = 10, zero_inflation = TRUE,
...
```{r, running-zimvnb}
zimvnb <- normal_block(data, blocks = 10, zero_inflation = TRUE,
                       control = NB_control(clustering_init = zimvnb_hclust$clustering,
...
```{r, zi-clustering}
cl_rho_hcvar <- cutree(hclustvar(zimvnb$var_par$rho), 10)
ARI_rho <- data.frame(
...
```{r, sparse-network}
zimvnb_sparse <- normal_block(data, blocks = as_indicator(zimvnb$clustering), zero_inflation = TRUE,
                              sparsity = 0.005,
                              control = NB_control(threshold = 1e-6, niter = 300, verbose = TRUE))
```

```{r, plot}
zimvnb_sparse$plot_network()
```
