...
test_that("NB_fixed_blocks_diagonal: check dimensions, optimization and field access", {
  model <- NB_fixed_blocks_diagonal$new(Y, X, C)
  model$optimize()
  ...
  model_sparse <- NB_fixed_blocks_diagonal$new(Y, X, C, sparsity = 0.2)
  model_sparse$optimize()
})

test_that("NB_fixed_blocks_spherical: check dimensions, optimization and field access", {
  model <- normalblockr:::NB_fixed_blocks_spherical$new(Y, X, C)
  model$optimize()
  params <- model$model_par
  expect_equal(model$n, nrow(Y))
  expect_equal(model$p, ncol(Y))
  ...
  model_sparse <- NB_fixed_blocks_spherical$new(Y, X, C, sparsity = 0.2)
  model_sparse$optimize()
})
