...
test_that("normal block with changing sparsity, integrated inference", {
  ...
  model <- NB_changing_sparsity$new(data, Q, control = NB_control(verbose = FALSE))
  model$optimize()
...
test_that("normal block with fixed clusters, spherical residual covariance and heuristuic inference", {
  data <- normal_data$new(Y, X)
  model <- NB_changing_sparsity$new(data, C, control = NB_control(noise_covariance = "spherical",
                                                                  heuristic = TRUE,
                                                                  verbose = FALSE))
  model$optimize()
  model_StARS <- model$get_best_model("StARS")
